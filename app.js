(function () {
  ...
  function applyFontSize() {
    ...
    document.querySelectorAll("[data-font-action='decrease']").forEach((b) => { b.disabled = sizeIndex === 0; });
    document.querySelectorAll("[data-font-action='increase']").forEach((b) => { b.disabled = sizeIndex === sizes.length - 1; });
  }

  function editionUrl(date, category) { return `edition.html?date=${date}${category ? `&category=${category}` : ""}`; }

  // ── Shared chrome (nav, footer) — uses index metadata only ──────────────────
  function renderShared() {
    document.querySelectorAll("[data-i18n]").forEach((el) => { el.innerHTML = dual(copy[el.dataset.i18n]); });
    const latest = news.editions[0];
    const nav = document.getElementById("nav-links");
    if (nav) nav.innerHTML = `<a class="active" href="index.html">${inlineDual("首頁", "Front Page")}</a>` + news.categories.slice(1).map((c) => `<a href="${editionUrl(latest.date, c.id)}">${inlineDual(c.zh, c.en)}</a>`).join("");
    const footer = document.getElementById("footer-sections");
    if (footer) footer.innerHTML = news.categories.slice(1).map((c) => `<a href="${editionUrl(latest.date, c.id)}">${inlineDual(c.zh, c.en)}</a>`).join("");
    const todayLabel = document.getElementById("today-label");
    if (todayLabel) todayLabel.innerHTML = dateDual(new Date().toISOString().slice(0, 10), true);
  }

  // ── Archive grid — uses index metadata only (no stories needed) ──────────────
  function renderArchive() {
    const grid = document.getElementById("archive-grid");
    if (!grid) return;
    grid.innerHTML = news.editions.map((ed, i) =>
      `<a class="archive-card archive-${ed.color}" href="${editionUrl(ed.date)}">` +
      `<span class="archive-number">${String(i + 1).padStart(2, "0")}</span>` +
      `<span class="archive-date">${dateDual(ed.date, true)}</span>` +
      `<strong>${dual(ed.theme)}</strong>` +
      `<div class="archive-summary">${dual(ed.summary)}</div>` +
      `<span class="archive-link">${inlineDual("閱讀本期", "Open edition")} →</span>` +
      `</a>`
    ).join("");
  }

  // ── Home page story sections — need today's stories ──────────────────────────
  function storyMarkup(story, compact) {
    const category = news.categories.find((c) => c.id === story.category);
    ...
    if (compact) return (
      ...
      `<p class="story-category">${inlineDual(category.zh, category.en)}</p>` +
      `<h3><a href="${url}">${dual(story.title)}</a></h3>` +
      `<div class="story-summary">${dual(story.dek)}</div>` +
      `<div class="story-meta"><span>${meta}</span></div>` +
      `</article>`
    );
    return (
      `<a class="story-image image-${story.image}" href="${url}"><span class="image-credit">Daily News / Staff</span></a>` +
      `<div class="lead-copy">` +
      `<p class="story-category">${inlineDual(category.zh, category.en)} · ${inlineDual("焦點報導", "Lead Story")}</p>` +
      `<h2><a href="${url}">${dual(story.title)}</a></h2>` +
      `<div class="dek">${dual(story.dek)}</div>` +
      `<div class="story-meta"><span>${inlineDual(`記者 ${story.author}`, `By ${story.author}`)}</span><span>${meta}</span></div>` +
      `</div>`
    );
  }
  ...
  function renderHomeStories() {
    ...
    const stories = (window.DAILY_NEWS_STORIES || {})[latest.date];
    if (!stories) {
      lead.innerHTML = `<p class="loading-msg">${dual(copy.loading)}</p>`;
      return;
    }

    const featured = stories.filter((s) => s.lead);
    document.getElementById("current-date").innerHTML = dateDual(latest.date);
    ["today-link", "banner-link", "breaking-link"].forEach((id) => {
      const el = document.getElementById(id);
      if (el) el.href = editionUrl(latest.date);
    });
    lead.innerHTML = storyMarkup(featured[0], false);
    document.getElementById("home-side").innerHTML = featured.slice(1, 4).map((s) => storyMarkup(s, true)).join("");
    document.getElementById("desk-grid").innerHTML = news.categories.slice(1).map((cat) => {
      const story = stories.find((s) => s.category === cat.id);
      if (!story) return "";
      return `<a class="desk-card" href="${editionUrl(latest.date, cat.id)}">` +
             `<span>${inlineDual(cat.zh, cat.en)}</span>` +
             `<strong>${dual(story.title)}</strong>` +
             `<div class="desk-summary">${dual(story.dek)}</div>` +
             `<b>→</b></a>`;
    }).join("");
  }

  // ── Lazy-load the monthly stories file for today's edition ───────────────────
  ...
  function loadMonthScript(yyyyMM, onReady) {
    ...
    // Already loaded by a previous call this session — fire immediately
    const alreadyLoaded = document.querySelector(`script[src="${src}"]`);
    if (alreadyLoaded) {
      const stories = (window.DAILY_NEWS_STORIES || {})[news.editions[0].date];
      if (stories) { onReady(); return; }
    }

    // Listen for the custom event dispatched at the bottom of each monthly file
    document.addEventListener("dn:stories-loaded", onReady, { once: true });

    // Inject the <script> tag; browser fetches and executes it once
    const tag = document.createElement("script");
    tag.src = src;
    tag.onerror = () => { console.error(`[DailyNews] Could not load ${src}`); };
    document.head.appendChild(tag);
  }

  // ── Init ─────────────────────────────────────────────────────────────────────
  document.querySelectorAll("[data-font-action]").forEach((b) =>
    b.addEventListener("click", () => { sizeIndex += b.dataset.fontAction === "increase" ? 1 : -1; applyFontSize(); })
  );
  ...
  applyFontSize();
  renderShared();   // nav + footer — instant, no stories needed
  renderArchive();  // archive grid — instant, no stories needed

  // Home page stories load after the monthly file arrives
  if (document.getElementById("home-lead")) {
    const latest = news.editions[0];
    loadMonthScript(monthKey(latest.date), renderHomeStories);
  }
})();
